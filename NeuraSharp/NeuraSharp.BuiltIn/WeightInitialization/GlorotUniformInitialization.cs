...
namespace NeuraSharp.BuiltIn.WeightInitialization
{
    ...
    public class GlorotUniformInitialization<T> : IWeightInitialization<T> where T : INumber<T>, IFloatingPointIeee754<T>
    {
        public GlorotUniformInitialization()
        {
        }
        ...
        public void Initialize(IWeightsLayer<T> weights)
        {
            for(int l = 0; weights.Weights.Length > l; l++)
            {
                ...
                T sq = T.Sqrt(T.CreateChecked( 6.0 /(weights.Weights[l].Length + weights.Weights.Length)) );
                T min = -sq;
                T max = sq;
