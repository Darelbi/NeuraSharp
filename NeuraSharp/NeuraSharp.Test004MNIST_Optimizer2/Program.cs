...
// This is the second attemp to optimize, after the first 50 tries I found out
// there was no netork performing well using LeakyREUL or Sigmoid (wrong assumption)
// so i try a random optimization again AND i remove those 2 activation functions from possibilities
// also only the networks with 1-3 hidden layers performs well. Also only 25-81 neurons works as number
// so i reduce the possibilities here too. tweked also learning rate too. and now increased epochs
...
using (StreamWriter writer = new StreamWriter("Compare random nets.csv"))
{
    for (int mmm = 0; mmm < 50; mmm++)
    {
        ...
        int[] numNeuronsLookup = [25, 36, 49, 64, 81, 100];
        ...
        for (int i = 0; i < numNeurons.Count - 1; i++)
        {
            ...
            int layerChoice = Random.Shared.Next(0, 3);
            switch (
                layerChoice)
            {
                case 0:
        ...
        float huberLoss = (float)Random.Shared.NextDouble() * 0.3f + 0.6f;
        ...
        float LearningRate = (float)(Random.Shared.NextDouble() * 0.0007 + 0.0004);
        ...
        for (int i = 0; i < epochs; i++)
        {
            ...
            if (i % 300 == 0)
            {
                ...
                accuracy = ((total - wrong) / (float)total);
